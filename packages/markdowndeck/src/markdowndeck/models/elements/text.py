...
class TextElement(Element):
    ...
    def split(
        ...
        REFACTORED: This method is now self-consistent and robust against padding issues.
        """
        logger.debug(
        ...
        # FIXED: This block handles the core logic for the off-by-one line bug.
        # It ensures that even if padding is large, we try to fit content.
        if content_height_available <= 0 and available_height > 0:
            logger.warning(
                f"Padding ({vertical_padding:.2f}) exceeds available height ({available_height:.2f}). "
                f"Ignoring padding for fitted part to fit minimal content."
            )
            content_height_available = available_height
            vertical_padding = 0  # The fitted part will have no padding.
            logger.debug(
                f"Revised content_height_available to {content_height_available:.2f} and vertical_padding to 0."
            )
        ...
        overflowing_part.object_id = None
