...
def calculate_list_element_height(element: ListElement | dict, available_width: float) -> float:
    ...
    final_height = max(total_height, 30.0)  # Ensure a minimum reasonable height for a non-empty list
    logger.debug(f"List ({list_element.element_type}) calculated height: {final_height:.2f} for width {available_width:.2f}")
    return final_height


def _calculate_single_item_total_height(item: ListItem, available_width: float, level: int, item_spacing: float) -> float:
    """
    ...
    item_text_width = max(10.0, available_width - current_item_indent - 8)  # Reduced from 10 (marker padding)
    ...
    if item.children:
        ...
        for child_item in item.children:
            children_height += _calculate_single_item_total_height(child_item, available_width, level + 1, item_spacing)
