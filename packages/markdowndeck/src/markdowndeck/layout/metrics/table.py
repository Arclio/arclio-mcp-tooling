...
def calculate_table_element_height(element: TableElement | dict, available_width: float) -> float:
    ...
    if table_element.headers:
        header_height = calculate_row_height(table_element.headers, col_width, is_header=True)
...
def calculate_row_height(row_data: list, col_width: float, is_header: bool = False) -> float:
    """
