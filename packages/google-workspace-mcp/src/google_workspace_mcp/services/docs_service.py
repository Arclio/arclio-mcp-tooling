...
class DocsService(BaseGoogleService):
    ...
    def insert_image(
        ...
        try:
            ...
            result = (
                self.service.documents().batchUpdate(documentId=document_id, body={"requests": [insert_request]}).execute()
            )
            ...
            logger.info(f"Successfully inserted image {inserted_image_id} into document {document_id}")
            ...
            logger.exception(f"Unexpected error inserting image into document {document_id}")
            return {
                "error": True,
                "error_type": "unexpected_service_error",
                "message": str(e),
