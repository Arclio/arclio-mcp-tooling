...
import pytest
from arclio_mcp_gsuite.tools.gmail import get_gmail_email
...
class TestGetGmailEmailTool:
    ...
    async def test_get_email_success(self, mock_gmail_service):
        ...
        mock_gmail_service.get_email_with_attachments.assert_called_once_with(
            "email123"
        )
    ...
    async def test_get_email_service_error_none(self, mock_gmail_service):
        ...
        with pytest.raises(
            ValueError, match="Failed to retrieve email with ID: error_none_id"
        ):
            await get_gmail_email(**args)
