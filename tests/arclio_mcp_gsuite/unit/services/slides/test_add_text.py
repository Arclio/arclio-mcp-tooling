...
class TestSlidesAddText:
    ...
    def test_add_text_success(self, mock_slides_service):
        ...
        call_args = (
            mock_slides_service.service.presentations.return_value.batchUpdate.call_args
        )
        ...
        assert (
            create_shape_req["elementProperties"]["transform"]["translateX"]
            == position[0]
        )
        assert (
            create_shape_req["elementProperties"]["transform"]["translateY"]
            == position[1]
        )
        assert (
            create_shape_req["elementProperties"]["size"]["width"]["magnitude"]
            == size[0]
        )
        assert (
            create_shape_req["elementProperties"]["size"]["height"]["magnitude"]
            == size[1]
        )
    ...
    def test_add_text_error(self, mock_slides_service):
        ...
        mock_slides_service.service.presentations.return_value.batchUpdate.return_value.execute.side_effect = (
            http_error
        )
