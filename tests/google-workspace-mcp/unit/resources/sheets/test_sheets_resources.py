...
class TestGetSpreadsheetMetadataResource:
    ...
    def mock_sheets_service_instance(self):
        ...
        with patch(
            "google_workspace_mcp.resources.sheets_resources.SheetsService"
    ...
    async def test_get_spreadsheet_metadata_success(self, mock_sheets_service_instance):
        ...
        result = await get_spreadsheet_metadata_resource(
    ...
    async def test_get_spreadsheet_metadata_empty_id(self):
        ...
        with pytest.raises(
            ValueError, match="Spreadsheet ID is required in the URI path"
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_spreadsheet_metadata_service_error(
    ...
    @pytest.mark.asyncio
    async def test_get_spreadsheet_metadata_none_result(
        self, mock_sheets_service_instance
    ):
        ...
        with pytest.raises(
            ValueError, match="Could not retrieve metadata for spreadsheet ID"
        ):
...
class TestGetSpecificSheetMetadataResource:
    ...
    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_by_id_success(
        self, mock_sheets_service_instance
    ):
        ...
        result = await get_specific_sheet_metadata_resource(
            spreadsheet_id="test_sheet_123", sheet_identifier="123456789"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_by_title_success(
        self, mock_sheets_service_instance
    ):
        ...
        result = await get_specific_sheet_metadata_resource(
            spreadsheet_id="test_sheet_123", sheet_identifier="Data Sheet"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_case_insensitive_title(
        self, mock_sheets_service_instance
    ):
    ...
    async def test_get_specific_sheet_metadata_empty_spreadsheet_id(self):
        ...
        with pytest.raises(
            ...
            await get_specific_sheet_metadata_resource(
                spreadsheet_id="", sheet_identifier="Sheet1"
            )
    ...
    async def test_get_specific_sheet_metadata_empty_sheet_identifier(self):
        ...
        with pytest.raises(
            ...
            await get_specific_sheet_metadata_resource(
                spreadsheet_id="test_sheet_123", sheet_identifier=""
            )

    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_service_error(
        self, mock_sheets_service_instance
    ):
        ...
        with pytest.raises(ValueError, match="Spreadsheet not found"):
            await get_specific_sheet_metadata_resource(
                spreadsheet_id="nonexistent", sheet_identifier="Sheet1"
            )

    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_no_sheets(
        self, mock_sheets_service_instance
    ):
        ...
        with pytest.raises(ValueError, match="No sheets found in spreadsheet"):
            await get_specific_sheet_metadata_resource(
                spreadsheet_id="test_sheet_123", sheet_identifier="Sheet1"
            )

    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_sheet_not_found(
        self, mock_sheets_service_instance
    ):
        ...
        with pytest.raises(
            ValueError, match="Sheet 'NonexistentSheet' not found in spreadsheet"
        ):
            await get_specific_sheet_metadata_resource(
                spreadsheet_id="test_sheet_123", sheet_identifier="NonexistentSheet"
            )

    @pytest.mark.asyncio
    async def test_get_specific_sheet_metadata_none_result(
        self, mock_sheets_service_instance
    ):
