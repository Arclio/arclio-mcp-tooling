...
class TestLayoutCalculatorIntegration:
    ...
    def test_universal_section_model_content_aware_sizing(self, layout_manager: LayoutManager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Test Title", object_id="title_1")
        ...
        footer = TextElement(element_type=ElementType.FOOTER, text="Page Footer", object_id="footer_1")
        ...
        body_element_count = len(
            [e for e in slide.elements if e.element_type not in (ElementType.TITLE, ElementType.SUBTITLE, ElementType.FOOTER)]
        )
        assert len(root_section.elements) == body_element_count, "Root section should contain all body elements"

        # Verify header/footer elements (in top-level elements) have positions and sizes
        header_footer_elements = [
            e for e in result_slide.elements if e.element_type in (ElementType.TITLE, ElementType.SUBTITLE, ElementType.FOOTER)
        ]
        for element in header_footer_elements:
            assert element.position is not None, f"Header/footer element {element.object_id} missing position"
            assert element.size is not None, f"Header/footer element {element.object_id} missing size"
            assert element.size[0] > 0, f"Header/footer element {element.object_id} has invalid width"
            assert element.size[1] > 0, f"Header/footer element {element.object_id} has invalid height"

        # Verify body elements (in sections) have positions and sizes
        for section in result_slide.sections:
            for element in section.elements:
                assert element.position is not None, f"Section element {element.object_id} missing position"
                assert element.size is not None, f"Section element {element.object_id} missing size"
                assert element.size[0] > 0, f"Section element {element.object_id} has invalid width"
                assert element.size[1] > 0, f"Section element {element.object_id} has invalid height"

        # Get positioned elements from the root section
        section_elements = root_section.elements
        positioned_short = next(e for e in section_elements if e.object_id == "short_text")
        positioned_long = next(e for e in section_elements if e.object_id == "long_text")
        positioned_list = next(e for e in section_elements if e.object_id == "simple_list")

        # Get header/footer elements from slide elements
        positioned_title = next(e for e in result_slide.elements if e.object_id == "title_1")
        positioned_footer = next(e for e in result_slide.elements if e.object_id == "footer_1")

        # Verify content-aware sizing: longer content should result in taller elements
        assert positioned_long.size[1] > positioned_short.size[1], (
            f"Longer text ({positioned_long.size[1]}) should be taller than shorter text ({positioned_short.size[1]})"
        )

        assert positioned_list.size[1] > positioned_short.size[1], "List should be taller than simple text"
        ...
        for i in range(len(section_elements_list) - 1):
            ...
            assert spacing <= VERTICAL_SPACING * 2, f"Spacing too large between elements {i} and {i + 1}"
        ...
        footer_zone_top = DEFAULT_SLIDE_HEIGHT - layout_manager.margins["bottom"] - FOOTER_HEIGHT

        # Title in header zone
        assert positioned_title.position[1] >= layout_manager.margins["top"], "Title should be in header zone"
        assert positioned_title.position[1] < header_zone_bottom, "Title should be within header zone"

        # Root section should be positioned in body zone
        assert root_section.position[1] >= body_zone_top, "Root section should be in body zone"

        # Footer in footer zone
        assert positioned_footer.position[1] >= footer_zone_top, "Footer should be in footer zone"

    def test_explicit_sections_preserved_and_positioned(self, layout_manager: LayoutManager):
        """Test that explicitly defined sections are preserved and positioned correctly."""

        title = TextElement(element_type=ElementType.TITLE, text="Explicit Sections Test")

        text1 = TextElement(element_type=ElementType.TEXT, text="Left content", object_id="left_text")
        text2 = TextElement(element_type=ElementType.TEXT, text="Right content", object_id="right_text")
        ...
        assert abs(left_sec.size[0] / body_width - 0.7) < 0.1, "Left section should be ~70% width"
        assert abs(right_sec.size[0] / body_width - 0.3) < 0.1, "Right section should be ~30% width"

        # Verify horizontal arrangement
        assert left_sec.position[0] < right_sec.position[0], "Left section should be to left of right"
    ...
    def test_section_equal_division_no_directives(self, layout_manager: LayoutManager):
        ...
        section1 = Section(
            id="equal_section_1",
            type="section",
            elements=[TextElement(element_type=ElementType.TEXT, text="Content 1", object_id="text_1")],
        )
        section2 = Section(
            id="equal_section_2",
            type="section",
            elements=[TextElement(element_type=ElementType.TEXT, text="Content 2", object_id="text_2")],
        )
        section3 = Section(
            id="equal_section_3",
            type="section",
            elements=[TextElement(element_type=ElementType.TEXT, text="Content 3", object_id="text_3")],
        )
    ...
    def test_section_explicit_directive_splits(self, layout_manager: LayoutManager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Explicit Division Test")
        ...
        slide = Slide(
            ...
            elements=[title] + [s.elements[0] for s in [left_section, middle_section, right_section]],
        ...
        for i, (section, expected_width) in enumerate(zip(sections, expected_widths, strict=False)):
            actual_width = section.size[0]
            assert (
                abs(actual_width - expected_width) < 2  # Small tolerance for floating point precision
            ), (
                f"Section {i} width {actual_width:.1f} should be approximately {expected_width:.1f} (usable_width={usable_width:.1f})"
            )

        # Verify horizontal arrangement
        assert sections[0].position[0] < sections[1].position[0] < sections[2].position[0], (
            "Sections should be arranged left to right"
        )

    def test_vertical_layout_with_height_directives(self, layout_manager: LayoutManager):
        ...
        text1 = TextElement(element_type=ElementType.TEXT, text="Top content", object_id="top_text")
        text2 = TextElement(
            element_type=ElementType.TEXT,
            text="Bottom content",
        ...
        assert abs(top_sec.size[1] - 100) < 5, f"Top section should have specified height, got {top_sec.size[1]}"
        assert abs(bottom_sec.size[1] - 80) < 5, f"Bottom section should have specified height, got {bottom_sec.size[1]}"

        # Verify both sections span full width (vertical layout)
        body_width = layout_manager.position_calculator.body_width
        assert abs(top_sec.size[0] - body_width) < 5, "Top section should span full width"
        assert abs(bottom_sec.size[0] - body_width) < 5, "Bottom section should span full width"

        # Verify vertical stacking
        assert top_sec.position[1] < bottom_sec.position[1], "Top section should be above bottom section"

        # Verify horizontal alignment (both should start at same X)
        assert abs(top_sec.position[0] - bottom_sec.position[0]) < 5, "Sections should align horizontally"
    ...
    def test_vertical_alignment_middle_and_bottom(self, layout_manager: LayoutManager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Vertical Alignment Test")

        # Create sections with different vertical alignments and fixed heights
        short_text1 = TextElement(element_type=ElementType.TEXT, text="Line 1", object_id="line1")
        short_text2 = TextElement(element_type=ElementType.TEXT, text="Line 2", object_id="line2")
        ...
        bottom_text1 = TextElement(element_type=ElementType.TEXT, text="Bottom 1", object_id="bottom1")
        bottom_text2 = TextElement(element_type=ElementType.TEXT, text="Bottom 2", object_id="bottom2")
        ...
        total_content_height = sum(e.size[1] for e in middle_elements) + VERTICAL_SPACING * (len(middle_elements) - 1)
        ...
        assert abs(actual_start_y - expected_start_y) < 10, (
        ...
        bottom_content_height = sum(e.size[1] for e in bottom_elements) + VERTICAL_SPACING * (len(bottom_elements) - 1)
        ...
        expected_bottom_start_y = bottom_section_top + bottom_section_height - bottom_content_height
        actual_bottom_start_y = bottom_elements[0].position[1]

        assert abs(actual_bottom_start_y - expected_bottom_start_y) < 10, (
    ...
    def test_overflow_acceptance_no_constraints(self, layout_manager: LayoutManager):
        ...
        assert table_element.position is not None, "Overflowing table should still be positioned"
        ...
        assert table_element.size[1] > section.size[1], (
        ...
        assert table_element.position[0] >= section.position[0], "Table should start within section horizontally"
        assert table_element.position[1] >= section.position[1], "Table should start within section vertically"
    ...
    def test_complex_nested_section_layout(self, layout_manager: LayoutManager):
        ...
        # Main row container
        main_row = Section(id="main_row", type="row", subsections=[left_column, right_column])
        ...
        assert left_col_result.position[0] < right_col_result.position[0], "Left column should be to the left of right column"
        ...
        assert abs(left_col_result.size[0] - expected_left_width) < 20, (
            f"Left column width should be ~60% of body width, got {left_col_result.size[0]}"
        )
        assert abs(right_col_result.size[0] - expected_right_width) < 20, (
        ...
        assert right_top_result.position[1] < right_bottom_result.position[1], (
        ...
        assert abs(right_top_result.size[1] - expected_top_height) < 10, (
            "Right top section height should be ~30% of right column height"
        )
        assert abs(right_bottom_result.size[1] - expected_bottom_height) < 10, (
            "Right bottom section height should be ~70% of right column height"
        )

    def test_intrinsic_element_heights_universal_model(self, layout_manager: LayoutManager):
        """Test that the universal model respects intrinsic element heights from content."""

        title = TextElement(element_type=ElementType.TITLE, text="Intrinsic Height Test")
        ...
        large_list = ListElement(
            element_type=ElementType.BULLET_LIST,
            items=[ListItem(text=f"Item {i} with substantial content that will take up space") for i in range(1, 8)],
        ...
        positioned_empty = next(e for e in root_section.elements if e.object_id == "empty_text")
        positioned_minimal = next(e for e in root_section.elements if e.object_id == "minimal_text")
        positioned_code = next(e for e in root_section.elements if e.object_id == "medium_code")
        positioned_list = next(e for e in root_section.elements if e.object_id == "large_list")

        # Verify elements have different heights based on content
        assert positioned_list.size[1] > positioned_code.size[1], "Large list should be taller than medium code block"

        assert positioned_code.size[1] > positioned_minimal.size[1], (
            "Multi-line code should be taller than single character text"
        )

        assert positioned_minimal.size[1] >= positioned_empty.size[1], (
        ...
        for element in [
            ...
            assert element.size[1] < 1000, f"Element height should be reasonable, got {element.size[1]}"
        ...
        for i in range(len(section_elements) - 1):
            current_bottom = section_elements[i].position[1] + section_elements[i].size[1]
            next_top = section_elements[i + 1].position[1]
            assert next_top >= current_bottom, f"Element {i + 1} should not overlap element {i}"
    ...
    def test_unified_spacing_consistency(self, layout_manager: LayoutManager):
        ...
        for result_slide, test_name in [
            ...
            pos_heading = next(e for e in section.elements if e.object_id == "section_heading")
            pos_related = next(e for e in section.elements if e.object_id == "related_list")
            pos_unrelated1 = next(e for e in section.elements if e.object_id == "unrelated1")
            pos_unrelated2 = next(e for e in section.elements if e.object_id == "unrelated2")

            # Calculate spacing between related elements
            related_spacing = pos_related.position[1] - (pos_heading.position[1] + pos_heading.size[1])

            # Calculate spacing between unrelated elements
            unrelated_spacing = pos_unrelated2.position[1] - (pos_unrelated1.position[1] + pos_unrelated1.size[1])

            # Related elements should have smaller spacing in both cases
            assert related_spacing < unrelated_spacing, (
    ...
    def test_sequential_vertical_positioning(self, layout_manager: LayoutManager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Sequential Positioning Test")
        ...
        tall_section = Section(
            ...
            elements=[
                ListElement(
                    element_type=ElementType.BULLET_LIST,
                    items=[ListItem(text=f"Long item {i} with substantial content") for i in range(1, 10)],
                    object_id="tall_list",
                )
            ],
        )
        ...
        slide = Slide(
            ...
            elements=[title] + [s.elements[0] for s in [short_section, tall_section, medium_section]],
        ...
        assert sections[0].position[1] < sections[1].position[1] < sections[2].position[1], (
        ...
        assert tall_height > medium_height, "Section with many list items should be taller than code section"
        ...
        for i in range(len(sections) - 1):
            ...
            assert spacing <= VERTICAL_SPACING * 2, f"Spacing between sections {i} and {i + 1} should be reasonable"
