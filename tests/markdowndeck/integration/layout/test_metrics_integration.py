...
class TestMetricsSystemIntegration:
    ...
    def test_metrics_dispatch_correctness(self, layout_manager):
        ...
        list_elem = ListElement(element_type=ElementType.BULLET_LIST, items=[ListItem(text="Item")])
        table_elem = TableElement(element_type=ElementType.TABLE, headers=["H1"], rows=[["R1"]])
        code_elem = CodeElement(element_type=ElementType.CODE, code="print('test')")
        image_elem = ImageElement(element_type=ElementType.IMAGE, url="https://example.com/test.jpg")
        ...
        assert abs(text_height_main - text_height_direct) < 0.1, "Text metrics dispatch should match direct call"
        ...
        assert abs(list_height_main - list_height_direct) < 0.1, "List metrics dispatch should match direct call"

        table_height_main = calculate_element_height(table_elem, available_width)
        table_height_direct = calculate_table_element_height(table_elem, available_width)
        assert abs(table_height_main - table_height_direct) < 0.1, "Table metrics dispatch should match direct call"
        ...
        assert abs(code_height_main - code_height_direct) < 0.1, "Code metrics dispatch should match direct call"

        image_height_main = calculate_element_height(image_elem, available_width)
        image_height_direct = calculate_image_element_height(image_elem, available_width, 0)
        assert abs(image_height_main - image_height_direct) < 0.1, "Image metrics dispatch should match direct call"
    ...
    def test_metrics_minimum_heights_enforced(self, layout_manager):
        ...
        assert text_height >= MIN_TEXT_HEIGHT, f"Empty text should get minimum height {MIN_TEXT_HEIGHT}, got {text_height}"

        list_height = calculate_list_element_height(empty_list, available_width)
        assert list_height >= MIN_LIST_HEIGHT, f"Empty list should get minimum height {MIN_LIST_HEIGHT}, got {list_height}"

        table_height = calculate_table_element_height(empty_table, available_width)
        assert table_height >= MIN_TABLE_HEIGHT, (
        ...
        assert code_height >= MIN_CODE_HEIGHT, f"Empty code should get minimum height {MIN_CODE_HEIGHT}, got {code_height}"

        image_height = calculate_image_element_height(empty_image, available_width)
        assert image_height >= MIN_IMAGE_HEIGHT, (
    ...
    def test_metrics_width_responsiveness(self, layout_manager):
        ...
        wrappable_list = ListElement(
            ...
            items=[
                ...
                ListItem(text="Another bullet point with more content to test wrapping behavior"),
            ],
        )
        ...
        assert text_narrow > text_wide, f"Text should be taller when narrow ({text_narrow}) vs wide ({text_wide})"
        ...
        assert list_narrow > list_wide, f"List should be taller when narrow ({list_narrow}) vs wide ({list_wide})"
        ...
        assert code_narrow > code_wide, f"Code should be taller when narrow ({code_narrow}) vs wide ({code_wide})"
    ...
    def test_metrics_integration_in_unified_layout_context(self, layout_manager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Metrics Integration Test")

        # Create elements with known content characteristics
        short_text = TextElement(element_type=ElementType.TEXT, text="Brief content.", object_id="short_text")
        ...
        large_table = TableElement(
            ...
            rows=[[f"Row {i} Content A", f"Row {i} Content B", f"Row {i} Content C"] for i in range(1, 8)],
        ...
        positioned_text = next(e for e in root_section.elements if e.object_id == "short_text")
        positioned_list = next(e for e in root_section.elements if e.object_id == "medium_list")
        positioned_table = next(e for e in root_section.elements if e.object_id == "large_table")

        # Heights should reflect content complexity: table > list > short text
        assert positioned_table.size[1] > positioned_list.size[1], "Large table should be taller than medium list"
        assert positioned_list.size[1] > positioned_text.size[1], "Medium list should be taller than short text"
    ...
    def test_metrics_with_directives_and_constraints(self, layout_manager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Metrics Directives Test")
        ...
        positioned_large_font = next(e for e in root_section.elements if e.object_id == "large_font")
        positioned_narrow = next(e for e in root_section.elements if e.object_id == "narrow_text")
        positioned_image = next(e for e in root_section.elements if e.object_id == "fixed_image")

        # Large font text should be taller than normal
        normal_text = TextElement(element_type=ElementType.TEXT, text="Text with large font size")
        normal_height = calculate_text_element_height(normal_text, positioned_large_font.size[0])
        assert positioned_large_font.size[1] > normal_height, "Large font text should be taller than normal font text"

        # Narrow text should be taller due to wrapping
        wide_text = TextElement(element_type=ElementType.TEXT, text=narrow_text.text)
        wide_height = calculate_text_element_height(wide_text, layout_manager.position_calculator.body_width)
        assert positioned_narrow.size[1] > wide_height, "Narrow text should be taller due to wrapping"

        # Fixed image should have exact specified height
        assert abs(positioned_image.size[1] - 150) < 1, "Image with height directive should have exact specified height"
    ...
    def test_metrics_consistency_in_unified_model(self, layout_manager):
        ...
        root_slide = Slide(object_id="root_section_slide", elements=[test_text, test_list])

        # Test 2: Explicit section
        test_section = Section(id="test_section", type="section", elements=[test_text, test_list])
        ...
        explicit_text = next(e for e in explicit_section.elements if e.object_id == "test_text")
        explicit_list = next(e for e in explicit_section.elements if e.object_id == "test_list")

        # Heights should be very similar (allowing for small differences in available width)
        assert abs(root_text.size[1] - explicit_text.size[1]) < 5, (
            f"Text height should be consistent: root={root_text.size[1]}, explicit={explicit_text.size[1]}"
        )

        assert abs(root_list.size[1] - explicit_list.size[1]) < 5, (
    ...
    def test_metrics_with_section_constraints(self, layout_manager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Section Constraints Test")
        ...
        assert narrow_text.size[1] > wide_text.size[1], (
        ...
        assert narrow_text.size[0] < wide_text.size[0], "Narrow section text should be narrower"
    ...
    def test_metrics_error_handling_and_edge_cases(self, layout_manager):
        ...
        assert fallback_height_2 > 0, "Should return fallback height for invalid element"
    ...
    def test_metrics_performance_characteristics(self, layout_manager):
        ...
        assert calculation_time < 2.0, f"Layout calculation should complete quickly, took {calculation_time:.2f} seconds"
        ...
        expected_body_count = len(elements)  # All elements are body elements (no title/footer)
        assert len(root_section.elements) == expected_body_count, (
    ...
    def test_metrics_with_nested_sections(self, layout_manager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Nested Sections Metrics Test")
        ...
        assert len(nested_sec.subsections) == 2, "Nested section should have two subsections"
        ...
        assert abs(main_element.size[0] - expected_section_width) < 20, (
            "Main element width should be constrained by section width"
        )
        assert abs(inner1_element.size[0] - expected_section_width) < 20, (
    ...
    def test_metrics_integration_with_overflow_scenarios(self, layout_manager):
        ...
        title = TextElement(element_type=ElementType.TITLE, text="Overflow Metrics Test")
        ...
        assert abs(table_element.size[0] - section.size[0]) < 5, "Table width should be constrained by section width"
