...
def _find_request_by_id(requests: list, object_id: str, request_type: str) -> dict | None:
    ...
    return next(
        (r for r in requests if request_key in r and r[request_key].get("objectId") == object_id),
        None,
    )
...
def _get_shape_id_by_text(requests: list, text_substring: str) -> str | None:
    ...
    text_req = next(
        (r for r in requests if "insertText" in r and text_substring in r["insertText"]["text"]),
        None,
    )
    return text_req["insertText"]["objectId"] if text_req else None


def _find_shape_properties_with_background(requests: list, object_id: str) -> dict | None:
...
def _find_shape_properties_with_content_alignment(requests: list, object_id: str) -> dict | None:
    ...
    return next(
        (
            ...
            if "updateShapeProperties" in r
            and r["updateShapeProperties"].get("objectId") == object_id
            and "contentAlignment" in r["updateShapeProperties"].get("shapeProperties", {})
        ),
        None,
    )
...
class TestEndToEndFunctions:
    ...
    def test_e2e_f_01_simple_presentation(self, mock_api_client_cls: MagicMock):
        ...
        # Act
        result = create_presentation(markdown, title="Simple E2E Test", credentials=credentials)
    ...
    def test_e2e_f_02_overflow_handling(self, mock_api_client_cls: MagicMock):
        ...
        # Act
        create_presentation(markdown, title="Overflow E2E Test", credentials=credentials)

        # Assert
        mock_api_instance.create_presentation_from_deck.assert_called_once()
        passed_deck = mock_api_instance.create_presentation_from_deck.call_args[0][0]
        assert len(passed_deck.slides) > 1, "Overflow should have resulted in multiple slides."
        for slide in passed_deck.slides:
            assert isinstance(slide, Slide)
            assert slide.root_section is None, "Finalized slides must have root_section cleared"
            assert slide.renderable_elements, "Finalized slides must have renderable elements"
    ...
    def test_e2e_f_03_markdown_to_requests_blank_canvas(self):
        ...
        assert all(req["createShape"]["shapeType"] == "TEXT_BOX" for req in create_shape_reqs)
...
"""
        ...
        assert title_style_req["updateTextStyle"]["style"]["fontSize"]["magnitude"] == 40.0
        ...
        subtitle_shape_props = _find_shape_properties_with_background(requests, subtitle_id)
        assert subtitle_shape_props is not None, "Background directive from row was not applied to child element."
        bg_color = subtitle_shape_props["updateShapeProperties"]["shapeProperties"]["shapeBackgroundFill"]["solidFill"][
            "color"
        ]["rgbColor"]
        assert abs(bg_color["red"] - (0x11 / 255.0)) < 0.01

        # Color on subtitle text
        subtitle_text_style = _find_request_by_id(requests, subtitle_id, "updateTextStyle")
        subtitle_color = subtitle_text_style["updateTextStyle"]["style"]["foregroundColor"]["opaqueColor"]["rgbColor"]
        assert abs(subtitle_color["red"] - 1.0) < 0.01

        # Width on right column shape
        right_col_create_req = _find_request_by_id(requests, right_col_id, "createShape")
        assert (
            abs(right_col_create_req["createShape"]["elementProperties"]["size"]["width"]["magnitude"] - (720.0 / 3.0)) < 30.0
        )

        # Valign and Align on right column
        right_col_shape_props = _find_shape_properties_with_content_alignment(requests, right_col_id)
        assert right_col_shape_props["updateShapeProperties"]["shapeProperties"]["contentAlignment"] == "BOTTOM"
        right_col_para_style = _find_request_by_id(requests, right_col_id, "updateParagraphStyle")
        assert right_col_para_style["updateParagraphStyle"]["style"]["alignment"] == "END"
