        ...
        for slide in result_splitting[1:]:  # Check continuation slides
            ...
            def validate_reset_recursive(sections, path=""):
                for i, section in enumerate(sections):
                    ...
                    ), f"Section {path}[{i}] position should be reset"
                    assert (
                        section.size is None
                    ), f"Section {path}[{i}] size should be reset"

                    for j, element in enumerate(section.elements):
                        assert (
                            element.position is None
                        ), f"Element {path}[{i}].element[{j}] position should be reset"
                        assert (
                            element.size is None
                        ), f"Element {path}[{i}].element[{j}] size should be reset"
