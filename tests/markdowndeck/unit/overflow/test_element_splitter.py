...
class TestElementSplitMethods:
    ...
    def test_text_element_split_minimum_two_lines_requirement(self):
        ...
        if fitted and overflowing:
            ...
            assert overflowing_lines >= 1, "Overflowing part should have remaining lines"
            assert fitted_lines + overflowing_lines == original_lines, "Lines should add up to original"
        ...
        assert overflowing_small is not None, "Should return entire element as overflowing"
    ...
    def test_text_element_split_with_formatting_preservation(self):
        ...
        if fitted and overflowing:
            ...
            assert len(fitted.formatting) >= 0, "Fitted part should have appropriate formatting"
            assert len(overflowing.formatting) >= 0, "Overflowing part should have appropriate formatting"
            ...
            for fmt in overflowing.formatting:
                ...
                assert fmt.end <= len(overflowing.text), "Formatting end should be within text"
    ...
    def test_list_element_split_minimum_two_items_requirement(self):
        ...
        list_elem = ListElement(element_type=ElementType.BULLET_LIST, items=items, size=(400, 140))
        ...
        if fitted and overflowing:
            ...
            assert len(overflowing.items) >= 1, "Overflowing part should have remaining items"
            assert len(fitted.items) + len(overflowing.items) == len(list_elem.items), "Items should add up"
    ...
    def test_list_element_context_aware_continuation(self):
        ...
        if fitted and overflowing and hasattr(overflowing, "_continuation_title"):
            ...
            assert "Important Tasks" in continuation_title.text, "Should include original title"
            assert CONTINUED_ELEMENT_TITLE_SUFFIX in continuation_title.text, "Should include continuation suffix"
    ...
    def test_table_element_split_header_plus_two_rows_requirement(self):
        ...
        table = TableElement(element_type=ElementType.TABLE, headers=headers, rows=rows, size=(400, 160))
        ...
        if fitted and overflowing:
            ...
            assert len(overflowing.rows) >= 1, "Overflowing part should have remaining rows"
            assert len(fitted.rows) + len(overflowing.rows) == len(table.rows), "Rows should add up"
        ...
        assert fitted_small is None, "Should reject split when minimum not met"
        assert overflowing_small is not None, "Should return entire table as overflowing"
    ...
    def test_table_element_header_duplication_in_overflow(self):
        ...
        table = TableElement(element_type=ElementType.TABLE, headers=headers, rows=rows, size=(400, 200))
        ...
        if fitted and overflowing:
            ...
            assert overflowing.headers == headers, "Overflowing part should have duplicated headers"

            # Headers should be deep copies, not references
            assert fitted.headers is not overflowing.headers, "Headers should be independent copies"
    ...
    def test_code_element_language_preservation(self):
        ...
        if fitted and overflowing:
            assert fitted.language == "javascript", "Fitted part should preserve language"
            assert overflowing.language == "javascript", "Overflowing part should preserve language"

            # Verify both parts have valid code content
            assert len(fitted.code.strip()) > 0, "Fitted part should have code content"
            assert len(overflowing.code.strip()) > 0, "Overflowing part should have code content"
    ...
    def test_image_element_validation_methods(self):
        ...
        assert not data_image.is_web_image(), "Data image should not be identified as web image"
    ...
    def test_element_split_edge_cases_with_minimum_requirements(self):
        ...
        # Test 2: Empty list
        empty_list = ListElement(element_type=ElementType.BULLET_LIST, items=[], size=(400, 0))
    ...
    def test_split_method_polymorphism(self):
        ...
        for element in elements:
            ...
            assert hasattr(element, "split"), f"{element.element_type} should have split method"
            assert callable(element.split), f"{element.element_type}.split should be callable"

            # Split method should return tuple
            result = element.split(50.0)
            assert isinstance(result, tuple), f"{element.element_type}.split should return tuple"
            assert len(result) == 2, f"{element.element_type}.split should return 2-tuple"
    ...
    def test_table_element_structure_validation_methods(self):
        ...
        assert valid_table.get_row_count() == 3, "Should return correct row count (including header)"
        assert valid_table.requires_header_duplication(), "Should require header duplication"
        ...
        assert not no_header_table.requires_header_duplication(), "Should not require header duplication"
    ...
    def test_code_element_utility_methods(self):
        ...
        assert code_elem.get_display_language() == "Python", "Should map language correctly"

        # Test with unmapped language
        code_elem.language = "customlang"
        assert code_elem.get_display_language() == "Customlang", "Should capitalize unknown language"
