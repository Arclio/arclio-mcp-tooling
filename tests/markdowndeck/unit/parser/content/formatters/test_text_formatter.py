...
class TestTextFormatter:
    ...
    @pytest.fixture
    def formatter(
        self, factory: ElementFactory, directive_parser: DirectiveParser
    ) -> TextFormatter:
    ...
    def test_cannot_handle_image_only_paragraphs(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_process_heading_with_section_styling(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_process_heading_with_directives(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
...
This text should be centered and Arial."""
    ...
    def test_process_paragraph_mixed_directive_content(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
...
Second paragraph with size."""
        ...
        assert (
            "First paragraph with color." in element.text
            or "Second paragraph with size." in element.text
        )

    def test_process_paragraph_with_formatting_and_directives(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
        ...
        italic_formats = [
            f for f in element.formatting if f.format_type == TextFormatType.ITALIC
        ]
        assert len(bold_formats) == 1
        assert len(italic_formats) == 1

    def test_process_paragraph_empty_after_directive_removal(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_extract_element_directives_from_text_multiline(
...
Second line with different alignment."""
    ...
    def test_extract_line_start_directives_p4(self, formatter: TextFormatter):
        ...
        directives, cleaned = formatter._extract_line_start_directives(
            "[color=blue][margin=5px] Content here"
        )
        ...
        directives2, cleaned2 = formatter._extract_line_start_directives(
            "Just normal content"
        )
        assert directives2 == {}
        assert cleaned2 == "Just normal content"

    def test_extract_element_directives_complex_scenarios(
        self, formatter: TextFormatter
    ):
...
Final content line."""
    ...
    def test_process_blockquote_with_directives(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_extract_clean_text_and_formatting(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_extract_text_from_cleaned_content(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
        ...
        text, formatting = formatter._extract_text_from_cleaned_content(
            cleaned_content, original_tokens[1]
        )

        assert text == "Bold text"  # Should be plain text, not markdown
        # Note: This method processes the cleaned markdown, so formatting extraction happens properly

    def test_is_image_only_paragraph(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_find_paragraph_close(
        ...
        close_index = formatter._find_paragraph_close(
            tokens, 1
        )  # Start from inline token
    ...
    def test_directive_inheritance_and_override(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_element_directive_override_section(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
    ...
    def test_process_unsupported_token_type(
        self, formatter: TextFormatter, md_parser: MarkdownIt
    ):
