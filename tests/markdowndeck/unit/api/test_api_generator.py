...
class TestApiRequestGenerator:
    ...
    def test_generate_element_requests_dispatch(self, generator: ApiRequestGenerator, sample_slide: Slide):
        ...
        generator.text_builder.generate_text_element_requests = MagicMock(return_value=[{"req_text": True}])
        generator.list_builder.generate_bullet_list_element_requests = MagicMock(return_value=[{"req_bullet_list": True}])
        generator.list_builder.generate_list_element_requests = MagicMock(
            return_value=[{"req_ordered_list": True}]
        )  # For ORDERED_LIST
        generator.media_builder.generate_image_element_requests = MagicMock(return_value=[{"req_img": True}])
        generator.code_builder.generate_code_element_requests = MagicMock(return_value=[{"req_code": True}])
        generator.table_builder.generate_table_element_requests = MagicMock(return_value=[{"req_table": True}])
        ...
        generator.list_builder.generate_bullet_list_element_requests.assert_called_once_with(list_el, sample_slide.object_id)
        ...
        generator.media_builder.generate_image_element_requests.assert_called_once_with(img_el, sample_slide.object_id)
        generator.code_builder.generate_code_element_requests.assert_called_once_with(code_el, sample_slide.object_id)
        generator.table_builder.generate_table_element_requests.assert_called_once_with(table_el, sample_slide.object_id)
