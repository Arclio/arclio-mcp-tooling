...
class TestLayoutManager:
    ...
    def test_layout_c_08_invalid_image_url(
        ...
        Spec: Verify that an invalid image URL results in a placeholder being used,
              which is then laid out with a non-zero size.
        """
        # Arrange
        image = ImageElement(
            element_type=ElementType.IMAGE,
            url="invalid.png",
            directives={"width": 300, "height": 200},
        )
        slide = _create_unpositioned_slide(elements=[image])
        ...
        # Assert
        # The URL should be replaced with a placeholder URL.
        assert (
            "placehold.co" in positioned_image.url
        ), "URL should be replaced with a placeholder."

        # The placeholder image must have a valid, non-zero size.
        assert positioned_image.size is not None, "Placeholder image must have a size."
        assert positioned_image.size[0] > 0, "Placeholder image width must be non-zero."
        assert (
            positioned_image.size[1] > 0
        ), "Placeholder image height must be non-zero."
    ...
    def test_layout_c_14_meta_element_directive_precedence(
        ...
        """Test Case: LAYOUT-C-14. Spec: Meta-element directive precedence order."""
        # Arrange: Create slide with base directives and title directives
        title_element = TextElement(element_type=ElementType.TITLE, text="Test Title")
        slide = _create_unpositioned_slide(elements=[title_element])
        slide.base_directives = {
            "color": {"type": "color", "value": {"type": "named", "value": "blue"}}
        }
        slide.title_directives = {
            "color": {"type": "color", "value": {"type": "named", "value": "red"}}
        }

        # Act
        positioned_slide = layout_manager.calculate_positions(slide)
        positioned_title = positioned_slide.renderable_elements[0]

        # Assert: Title directive (Level 2) overrides base directive (Level 1)
