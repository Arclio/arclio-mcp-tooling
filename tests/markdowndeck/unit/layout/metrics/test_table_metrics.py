...
class TestTableMetrics:
    ...
    def test_calculate_table_height_many_rows(self):
        ...
        element_few = TableElement(
            headers=headers, rows=few_rows, element_type=ElementType.TABLE
        )
        element_many = TableElement(
            headers=headers, rows=many_rows, element_type=ElementType.TABLE
        )
...
class TestTableElementSplitting:
    ...
    def test_table_requires_header_duplication(self):
        ...
        element_no_headers = TableElement(
            headers=[], rows=[["R1C1", "R1C2"]], element_type=ElementType.TABLE
        )
