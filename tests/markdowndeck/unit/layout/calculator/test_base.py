...
class TestBasePositionCalculator:
    ...
    def test_slide_zone_calculation_with_clear_spacing(self, calculator: PositionCalculator):
        ...
        expected_content_width = DEFAULT_SLIDE_WIDTH - DEFAULT_MARGIN_LEFT - DEFAULT_MARGIN_RIGHT
        expected_content_height = DEFAULT_SLIDE_HEIGHT - DEFAULT_MARGIN_TOP - DEFAULT_MARGIN_BOTTOM
        ...
        expected_body_top = calculator.header_top + HEADER_HEIGHT + HEADER_TO_BODY_SPACING
        ...
        # Verify footer zone
        expected_footer_top = DEFAULT_SLIDE_HEIGHT - DEFAULT_MARGIN_BOTTOM - FOOTER_HEIGHT
    ...
    def test_universal_section_model_root_section_creation(self, calculator: PositionCalculator):
        ...
        assert len(result_slide.sections) == 1, "Should have created exactly one root section"
        ...
        assert len(root_section.elements) == len(expected_body_elements), (
        ...
        for element in header_footer_elements:
            ...
            assert element.size is not None, f"Header/footer element {getattr(element, 'object_id', 'unknown')} not sized"

        # Verify body elements (in root section) have positions and sizes
        for element in root_section.elements:
            assert element.position is not None, f"Body element {getattr(element, 'object_id', 'unknown')} not positioned"
            assert element.size is not None, f"Body element {getattr(element, 'object_id', 'unknown')} not sized"

        # Get positioned elements from root section
        positioned_short = next(e for e in root_section.elements if e.object_id == "short_para")
        positioned_long = next(e for e in root_section.elements if e.object_id == "long_para")
        positioned_list = next(e for e in root_section.elements if e.object_id == "bullet_list")

        # Verify content-aware sizing
        assert positioned_long.size[1] > positioned_short.size[1], "Longer paragraph should be taller than shorter paragraph"

        assert positioned_list.size[1] > positioned_short.size[1], "List should be taller than short paragraph"
        ...
        assert positioned_title.position[1] >= calculator.header_top, "Title should be in header zone"
        assert positioned_title.position[1] < calculator.body_top, "Title should be within header zone"

        assert positioned_footer.position[1] >= calculator.footer_top, "Footer should be in footer zone"

    def test_universal_section_model_preserves_explicit_sections(self, calculator: PositionCalculator):
        ...
        assert abs(left_sec.size[0] / body_width - 0.7) < 0.1, (
            f"Left section should be ~70% width, got {left_sec.size[0] / body_width:.2f}"
        )
        assert abs(right_sec.size[0] / body_width - 0.3) < 0.1, (
        ...
        assert left_sec.position[0] < right_sec.position[0], "Left section should be to left of right section"

        # Verify both sections start at same Y coordinate (horizontal layout)
        assert abs(left_sec.position[1] - right_sec.position[1]) < 1, "Sections should be at same Y level"

        # Verify elements are positioned within their sections
        left_element = left_sec.elements[0]
        right_element = right_sec.elements[0]

        assert left_element.position[0] >= left_sec.position[0], "Left element should be within left section"
        assert right_element.position[0] >= right_sec.position[0], "Right element should be within right section"
    ...
    def test_empty_slide_universal_model(self, calculator: PositionCalculator):
        ...
        assert len(result_slide.sections) == 1, "Should create root section for empty slide"
    ...
    def test_slide_with_only_header_and_footer_universal_model(self, calculator: PositionCalculator):
        ...
        slide = Slide(object_id="header_footer_only_slide", elements=[title, footer], sections=[])
        ...
        root_section = result_slide.sections[0]
        assert len(root_section.elements) == 0, "Root section should be empty (no body elements)"
    ...
    def test_element_width_calculation_with_directives(self, calculator: PositionCalculator):
        ...
        assert abs(text_width - container_width) < 1, "Text should default to full container width"
        ...
        narrow_width = calculator._calculate_element_width(narrow_element, container_width)

        expected_narrow_width = container_width * 0.5
        assert abs(narrow_width - expected_narrow_width) < 1, (
        ...
        fixed_width = calculator._calculate_element_width(fixed_element, container_width)

        assert abs(fixed_width - 200) < 1, "Element should have specified absolute width"
        ...
        oversized_width = calculator._calculate_element_width(oversized_element, container_width)

        assert abs(oversized_width - container_width) < 1, "Oversized width directive should be clamped to container width"

    def test_body_elements_identification_universal_model(self, calculator: PositionCalculator):
        ...
        assert len(body_elements) == 3, f"Should have 3 body elements, got {len(body_elements)}"

        body_types = [e.element_type for e in body_elements]
        expected_types = [ElementType.TEXT, ElementType.TEXT, ElementType.CODE]

        assert body_types == expected_types, "Body elements should be text and code only"
        ...
        assert len(root_section.elements) == 3, "Root section should contain all body elements"
    ...
    def test_custom_slide_dimensions_and_margins(self):
        ...
        calculator = PositionCalculator(slide_width=custom_width, slide_height=custom_height, margins=custom_margins)
        ...
        expected_content_width = custom_width - custom_margins["left"] - custom_margins["right"]
        expected_content_height = custom_height - custom_margins["top"] - custom_margins["bottom"]
        ...
        assert abs(root_section.size[0] - expected_body_width) < 1, "Root section should use custom body width"

    def test_header_footer_positioning_universal_model(self, calculator: PositionCalculator):
        """Test that header and footer positioning works correctly in universal model."""

        title = TextElement(element_type=ElementType.TITLE, text="Test Title", object_id="title")
        subtitle = TextElement(
            element_type=ElementType.SUBTITLE,
            text="Test Subtitle",
            object_id="subtitle",
        )
        footer = TextElement(element_type=ElementType.FOOTER, text="Test Footer", object_id="footer")

        # Add some body content too
        body_text = TextElement(element_type=ElementType.TEXT, text="Body content", object_id="body")
        ...
        positioned_title = next(e for e in result_slide.elements if e.object_id == "title")
        positioned_subtitle = next(e for e in result_slide.elements if e.object_id == "subtitle")
        positioned_footer = next(e for e in result_slide.elements if e.object_id == "footer")

        # Verify header elements are in header zone
        assert positioned_title.position[1] >= calculator.header_top, "Title should be at or below header top"
        assert positioned_title.position[1] < calculator.body_top, "Title should be above body zone"

        assert positioned_subtitle.position[1] >= calculator.header_top, "Subtitle should be at or below header top"
        assert positioned_subtitle.position[1] < calculator.body_top, "Subtitle should be above body zone"

        # Subtitle should be below title
        assert positioned_subtitle.position[1] > positioned_title.position[1], "Subtitle should be positioned below title"
        ...
        root_section = result_slide.sections[0]
        assert len(root_section.elements) == 1, "Root section should contain body element"
        body_element = root_section.elements[0]
        assert body_element.object_id == "body", "Body element should be in root section"

    def test_horizontal_alignment_inheritance_universal_model(self, calculator: PositionCalculator):
        ...
        pos_left = next(e for e in root_section.elements if e.object_id == "left_aligned")
        pos_center = next(e for e in root_section.elements if e.object_id == "center_aligned")
        pos_right = next(e for e in root_section.elements if e.object_id == "right_aligned")
        ...
        # Left alignment
        assert abs(pos_left.position[0] - body_left) < 5, (
        ...
        assert abs(element_center - body_center) < 10, (
        ...
        assert abs(element_right - body_right) < 5, (
    ...
    def test_ensure_section_based_layout_method(self, calculator: PositionCalculator):
        ...
        body_text = TextElement(element_type=ElementType.TEXT, text="Body", object_id="body")

        slide_no_sections = Slide(object_id="no_sections", elements=[title, body_text], sections=[])
        ...
        assert len(sections) == 1, "Should create root section even with no body elements"
        root_section = sections[0]
        assert len(root_section.elements) == 0, "Root section should be empty"
